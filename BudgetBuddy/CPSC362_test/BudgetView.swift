            ...
            NavigationView {
                List(TYPES,id: \.description){ type in
                    Section(type) {
                        Chart {
                            BarMark(
                                x: .value("Amount", viewModel.transactions.sum(type)),
                                y: .value("", "")
                            )
                            .annotation{
                                Text("$\(viewModel.transactions.sum(type), specifier: "%.2f")")
                            }
                        ...
                        .padding(.horizontal)
                        .chartXScale(domain: [0,viewModel.transactions.sum(type)+viewModel.budget[type]!])
                        .frame(height: 50.0)
